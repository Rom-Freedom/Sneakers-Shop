<script setup>
  ...
  const createOrder = async () => {
    try {
      ...
      const {data } = await axios.post(`https://d6448e24801b37c1.mokky.dev/orders`, {
        items: cart.value,
  ...
  const addToFavorite = async (item) => {
    try {
        ...
        const { data } = await axios.post(`https://d6448e24801b37c1.mokky.dev/favorites`, obj)

        item.favoriteId = data.id
    } else {
      item.isFavorite = false
      await axios.delete(`https://d6448e24801b37c1.mokky.dev/favorites/${item.favoriteId}`)
